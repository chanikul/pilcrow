...
Remaining (Level 1): sub-tasks 8–10 (copy-HTML, share-URL, end-to-end test against `the-cheapest-signal`). Sub-task 8 is mechanically simplest (copy a snapshot of the typeset HTML to clipboard with inline `<style>` for portability). Sub-task 9 (share-URL) needed the settings shape, which sub-task 6 just locked. Sub-task 10 is the rigorous BrowserRenderer-vs-deployed comparison. Editorial-writer follow-up for the final invitation copy in the editor stand-in is still pending.
