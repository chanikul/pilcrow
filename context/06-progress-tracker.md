...
**Current goal:** Ship Level 1 of the playground sprint. Sub-tasks 4–8 live (page shell, editor, preview wired to BrowserRenderer, settings panel, copy-HTML action). Sub-tasks 9 (share-URL) and 10 (acceptance test against the-cheapest-signal) remain.
**Owner:** Chanikul
**Updated:** 2026-05-07
...
Remaining (Level 1): sub-tasks 9–10 (share-URL, end-to-end test against `the-cheapest-signal`). Sub-task 9 (share-URL) has the settings shape locked from sub-task 6 (`pilcrow:settings-changed` detail's `settings` field) and the editor markdown contract from sub-task 7 — encoder reads both, decoder dispatches `source: 'restore'` events. Sub-task 10 is the rigorous BrowserRenderer-vs-deployed comparison gate. Editorial-writer follow-up for the final invitation copy in the editor stand-in is still pending.
