...
- [2026-05-10] **shape-around lead-in pattern: load-bearing coupling between CSS and walker.** `.shape-obstacle { margin-top: 1.7em }` in `public/styles/global.css` and `SA_TOP_OFFSET = resolvedLineHeight` in `packages/pilcrow-typeset/src/playwright.ts` are paired constants. Both encode the same vertical offset: the float box starts one body line-height below the column top, and the walker subtracts the same offset from each line's local Y so silhouette-row lookups align with the float's actual position. If you change one, you must change the other. Setting `margin-top: 0` in CSS while leaving `SA_TOP_OFFSET` non-zero pushes line 1's narrowing up to where there is no obstacle; setting `SA_TOP_OFFSET = 0` while leaving `margin-top` non-zero pushes the silhouette lookup to row 0 of the silhouette while the float box starts at row `resolvedLineHeight`, and prose overlaps the figure's head. If body `line-height` ever changes from 1.7, both values must update. Candidate refactor when a third party needs to override the lead-in spacing: extract `--shape-around-top-offset` as a shared CSS custom property + JS constant.
